...
def get_downloader(progress_callback=None):
    ...
    namespace = {}
    exec(code_cache["download"], namespace)

    DownloaderClass = namespace.get("StandardDownloader")
    if not DownloaderClass:
        raise ImportError("StandardDownloader 未找到")

    return DownloaderClass(
        DOWNLOADS_DIR, wrapped_callback if progress_callback else None
    )
...
def do_extract():
    ...
    def task_thread():
        try:
            ...
            lic_resp = hub_request(
                "POST", "/api/license/request", {"client_id": CLIENT_ID}
            )
            print(f"[DEBUG] 许可证响应: {lic_resp}", flush=True)
            if not lic_resp.get("success"):
                raise PermissionError(lic_resp.get("reason", "许可证申请失败"))
    ...
    def heartbeat():
        while True:
            ...
            resp = hub_request(
                "POST",
                "/api/client/heartbeat",
                {"client_id": CLIENT_ID, "stats": client_stats, "no_messages": True},
            )
            if resp.get("success"):
                data = resp.get("data", {})
                new_key = data.get("config_key")
